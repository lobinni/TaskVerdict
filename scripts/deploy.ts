/**
 * Deploy TaskVerdict Intelligent Contract to GenLayer Bradbury Testnet
 * 
 * Usage:
 *   npx ts-node scripts/deploy.ts
 * 
 * Prerequisites:
 *   - GEN tokens in wallet (get from Discord faucet)
...
async function deploy() {
  console.log('🚀 Deploying TaskVerdict to GenLayer Bradbury Testnet...\n');
  ...
  if (!privateKey) {
    ...
    console.log('\nTo deploy, run:');
    console.log('  PRIVATE_KEY=your_private_key npx ts-node scripts/deploy.ts');
    console.log('\nOr use GenLayer Studio: https://studio.genlayer.com');
    process.exit(1);
  }
  ...
  try {
    ...
    if (contractAddress) {
      ...
      // Update genlayer.ts
      const genlayerPath = join(__dirname, '../src/genlayer.ts');
      let genlayerContent = readFileSync(genlayerPath, 'utf-8');
      genlayerContent = genlayerContent.replace(
        /export const CONTRACT_ADDRESS = '0x[0-9a-fA-F]+' as Address;/,
        `export const CONTRACT_ADDRESS = '${contractAddress}' as Address;`
      );
      writeFileSync(genlayerPath, genlayerContent);
      
      console.log('\n📝 Updated src/genlayer.ts with new contract address');
      console.log('\n🎉 Done! Run `npm run build` to build with the new contract address.');
    } else {
      console.log('\n⚠️ Contract may have deployed but address not found in receipt');
    ...
    if (error.message.includes('insufficient funds')) {
      console.log('\n💡 Tip: Get GEN tokens from the Discord faucet');
    }
    process.exit(1);
  }
}
